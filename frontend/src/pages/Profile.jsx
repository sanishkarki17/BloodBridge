...
export default function Profile() {
    ...
    const [requestFormData, setRequestFormData] = useState({
        name: '', blood_group: '', district: '', city: '', phone: '', hospital: ''
    });
    ...
    const fetchMyRequests = async () => {
        try {
            const res = await fetch('http://localhost:4000/api/profile/requests', {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await res.json();
            setMyRequests(data);
    ...
    const handleUpdateDonorInfo = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await fetch('http://localhost:4000/api/profile/donor', {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
    ...
    const handleDeleteDonorListing = async (donorId) => {
        if (!confirm('Are you sure you want to delete this donor listing? This will not delete your account.')) return;
        setLoading(true);
        try {
                    ...
                    <CardContent>
                        {!editing ? (
                            <div className="space-y-3">
                                ...
                                <Button variant="primary" size="sm" onClick={() => setEditing(true)} className="mt-4">
                                    Edit Profile
                                </Button>
                            </div>
                        ) : (
                        ...
                        <CardContent>
                            {!editingDonor ? (
                                profile?.donorInfo ? (
                                    <div className="space-y-3">
                                        <div>
                                            <span className="text-sm font-medium text-text-secondary">Blood Group:</span>
                                            <div className="mt-1">
                                                <Badge variant="blood">{profile.donorInfo.blood_group}</Badge>
                                            </div>
                                        </div>
                                        <div>
                                            <span className="text-sm font-medium text-text-secondary">Location:</span>
                                            <p className="text-text">{profile.donorInfo.city}, {profile.donorInfo.district}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <div className="mt-1">
                                                <Badge variant={profile.donorInfo.available ? 'available' : 'default'}>
                                                    {profile.donorInfo.available ? 'Available' : 'Unavailable'}
                                                </Badge>
                                            </div>
                                        </div>
                                        <div className="flex space-x-2 mt-4">
                                            <Button variant="primary" size="sm" onClick={() => setEditingDonor(true)}>
                                                Edit Donor Info
                                            </Button>
                                            <Button variant="danger" size="sm" onClick={() => handleDeleteDonorListing(profile.donorInfo.id)}>
                                                Delete Listing
                                            </Button>
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <p className="text-text-secondary mb-4">No donor information yet.</p>
                                        <Button variant="primary" size="sm" onClick={() => setEditingDonor(true)}>Add Donor Info</Button>
                                    </div>
                                )
                            ) : (
                                <form onSubmit={handleUpdateDonorInfo} className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <Input
                                            ...
                                            onChange={(e) => setDonorData({ ...donorData, district: e.target.value })}
                                        />
                                        <Input
                                            label="City"
                                    ...
                                    <div className="flex items-center">
                                        <input
                                            ...
                                            onChange={(e) => setDonorData({ ...donorData, available: e.target.checked })}
                                            className="mr-2"
                                        />
                                        <label className="text-sm text-text">Available for donation</label>
                                    </div>
                                    <div className="flex space-x-2">
                                        <Button type="submit" variant="primary" size="sm">Save Changes</Button>
                                        <Button type="button" variant="ghost" size="sm" onClick={() => setEditingDonor(false)}>Cancel</Button>
                                    </div>
                                </form>
                            )}
                        </CardContent>
                                            ...
                                            {editingRequest === req.id ? (
                                                ...
                                                <div className="flex items-start justify-between">
                                                    <div>
                                                        ...
                                                        <Badge variant="blood" className="mt-1">{req.blood_group}</Badge>
                                                        <p className="text-sm text-text-secondary mt-2">{req.city}, {req.district}</p>
                                                        <p className="text-sm text-text-secondary">{req.hospital}</p>
                                                    </div>
                                                    <div className="flex space-x-2">
                                                        <Button variant="outline" size="sm" onClick={() => handleEditRequest(req)}>Edit</Button>
